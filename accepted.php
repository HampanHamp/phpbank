...
<?php
$query="SELECT * FROM `phpb_transactions` WHERE `acceptor`='$accountid' AND `status`='1' ORDER BY `timestamp` DESC";
$result=mysql_query($query,$db);
...
<?php
$query="SELECT * FROM `phpb_transactions` WHERE `requester`='$accountid' AND `status`='1' ORDER BY `timestamp` DESC";
$result=mysql_query($query,$db);
while ($transactions=mysql_fetch_array($result))
{
